...
class DesignDocument(Document):
    ...
    def add_list(self, list_name, list_func):
        """
        Appends a list function to the locally cached DesignDocument
        indexes dictionary.

        :param list_name: Name used to identify the list.
        :param list_func: Javascript list function.
        """
        if self.get_list(list_name) is not None:
    ...
    def update_list(self, list_name, list_func):
        """
        ...
        :param list_func: Javascript list function.
        """
        if self.get_list(list_name) is None:
    ...
    def delete_list(self, list_name):
        """
        Removes an existing list in the locally cached DesignDocument
        lists dictionary.
    ...
    def fetch(self):
        ...
        if not self.indexes:
            ...
            self.setdefault('indexes', dict())
        if not self.lists:
            # Ensure lists dict exists in locally cached DesignDocument.
            self.setdefault('lists', dict())
    ...
    def save(self):
        ...
        if not self.lists:
            # Ensure empty lists dict is not saved remotely.
            self.__delitem__('lists')
    ...
    def iterlists(self):
        ...
        :returns: Iterable containing list name and associated
            list function
        """
    ...
    def get_list(self, list_name):
        """
        Retrieves a specific list from the locally cached DesignDocument
        lists dictionary by name.

        :param str list_name: Name used to identify the list.

        :returns: Index dictionary for the specified list name
        """
