...
class DesignDocumentTests(UnitTestDbBase):
    ...
    def test_adding_existing_list(self):
        """
    ...
    def test_update_a_list(self):
        ...
        """
        ddoc = DesignDocument(self.db, '_design/ddoc001')
        ddoc.add_list('list001', 'not-a-valid-list-function')
        self.assertEqual(
    ...
    def test_update_non_existing_list(self):
        """
    ...
    def test_fetch_list(self):
        ...
        list_func = ('function(head, req) { provides(\'html\', function() '
                     ...
                     '{ html += \'<li>\' + row.key + \':\' + row.value + \'</li>\\n\';} '
                     'html += \'</ol></body></html>\'; return html; }); }')
        ddoc.add_list('list001', list_func)
        ddoc.add_list('list002', list_func)
        ddoc.add_list('list003', list_func)
        ddoc.save()
        ddoc_remote = DesignDocument(self.db, '_design/ddoc001')
        self.assertNotEqual(ddoc_remote, ddoc)
        ddoc_remote.fetch()
    ...
    def test_save_with_no_lists(self):
        """
        Tests the functionality when saving a design document without a list function.
    ...
    def test_iterating_over_lists(self):
        """
    ...
    def test_listing_list_functions(self):
        """
        Test the retrieval of list functions list from DesignDocument.
        """
        ddoc = DesignDocument(self.db, '_design/ddoc001')
        list_func = ('function(head, req) { provides(\'html\', function() '
                     '{var html = \'<html><body><ol>\\n\'; while (row = getRow()) '
        ...
        ddoc.add_list('list003', list_func)
        self.assertTrue(
            all(x in ddoc.list_lists() for x in [
                'list001',
                'list002',
                'list003'
            ])
